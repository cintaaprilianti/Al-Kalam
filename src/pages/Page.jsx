...
function Page() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      ...
      try {
        ...
        const ayahData = pageAyahs.map((a) => ({
          arab: a.text,
        ...
        setError(`Gagal memuat halaman ${pageId}: ${err.message}`);
        setLoading(false);
      }
    };
  ...
  const fetchTranslation = async (languageCode) => {
    try {
      ...
      const res = await fetchWithRetry(`http://api.alquran.cloud/v1/page/${id}/${language.edition}`);
  ...
  const toggleBookmark = (ayahIndex) => {
    ...
    const exists = updated.find(
      (b) => b.page === id && b.globalAyahNumber === ayah.globalAyahNumber
    );
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-black text-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-300 font-light">Memuat halaman...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl shadow-xl p-6 mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm text-gray-400 font-light px depressed-3 py-1 bg-black bg-opacity-30 rounded-full">
              Halaman {id} • {ayahs.length} Ayat
            </span>
            <button
              onClick={() => navigate('/page')}
          ...
          <div className="flex flex-col items-center">
            <h1 className="text-3xl font-serif font-bold mb-4 text-white">Halaman {id}</h1>
        ...
        <div className="mt-10 flex justify-between items-center">
          <Link
            to={`/page/${Math.max(1, parseInt(id) - 1)}`}
          ...
          <Link
            to={`/page/${Math.min(604, parseInt(id) + 1)}`}
            className={`px-4 py-2 rounded-lg bg-gradient-to-br from-gray-800 to-gray-700 text-gray-300 hover:text-white transition-colors shadow-md flex items-center ${
              parseInt(id) >= 604 ? 'opacity-50 cursor-not-allowed' : ''
            }`}
            onClick={(e) => parseInt(id) >= 604 && e.preventDefault()}
          >
            Halaman Selanjutnya
            <svg
              xmlns="http://www.w3.org/2000/svg"
